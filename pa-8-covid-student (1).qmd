...
cases |> 
  ...
  mutate(date = date(date),
         month = month(date, label = , abbr = ),
         year = year(date)) |> 
  unite(date, month, year, sep = "/") |>
  group_by(date) %>% 
  summarise(sum = sum(cases_daily)) 
...
```{r}
#| label: ca-2021-covid
cases2021 = filter(cases, year(date) == "2021") |>
  filter(state == "California") |>
  mutate(month = month(date)) |>
  group_by(month) |>
  summarise(sum = sum(cases_daily))

cases2021_new = cases2021 |>
  mutate(cases2021, monthF = factor(month, levels = month)) |>
  mutate(months = c("Jan", "Feb", "Mar", "Apr", "May", "June", "July", "Aug", "Sep", "Oct", "Nov", "Dec")) |>
  mutate(months = factor(months, levels = months))
head(cases2021_new)
ggplot(data = cases2021_new, aes(x = months, y = sum)) +
  geom_bar(stat = 'identity', col = "black", fill = "steelblue") +
  labs(
    x = "Month",
    y = "Number of Cases",
    title = "COVID-19 cases by month in California for 2021"
  )
```
...
>The fewest cases in 2021 were in June.
...
```{r}
casesThree = filter(cases, year(date) == "2021") |>
  filter(state == "California" | state == "Oregon" | state == "Washington") |>
  mutate(month = month(date)) |>
  group_by(state, month) |>
  summarise(deathProp = sum(deaths_daily) / sum(cases_daily))

casesThree$deathProp = casesThree$deathProp * 100

casesCali = filter(casesThree, state == "California")
casesOre = filter(casesThree, state == "Oregon") 
casesWash = filter(casesThree, state == "Washington") 

print(casesThree)

ggplot(casesThree, aes(x = months, y = deathProp)) +
  geom_line(data = casesCali, aes(x = month, y = deathProp, col = "California")) +
  geom_line(data = casesOre, aes(x = month, y = deathProp, col = "Oregon")) +
  geom_line(data = casesWash, aes(x = month, y = deathProp, col = "Washington")) +
  labs(
    x = "Month",
    y = "Deaths",
    title = "Deaths per 100 cases of COVID-19 in California, Oregon, and Washington",
    subtitle = "Did different coastal states have different success rates of treating patients during the pandemic?"
  ) +
  scale_color_manual(name = "State", values = c("California" = "steelblue", "Oregon" = "firebrick", "Washington" = "darkorchid"))
```
